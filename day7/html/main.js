...
const years = inventors.reduce((acc, next) => acc + (next.passed - next.year), 0);
console.log(years);
...
/*
  diffenrent ways of transforming NodeList into the Array
  -- Array.prototype.slice.apply(NodeList);
  -- [].slice.apply(NodeList);
  -- Array.from(NodeList); ES6 Array method
  -- [...NodeList];  ES6 spread operator
*/
//const links = Array.prototype.slice.apply(category.querySelectorAll('a'));
//const de = links
...
const transportation = data.reduce(function(acc, nextItem){
  if (!acc[nextItem]) {
    acc[nextItem] = 0;
  }
  acc[nextItem]++;
...
// two ways to delete arr element
